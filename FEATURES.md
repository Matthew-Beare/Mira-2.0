...
**Why it exists / user outcome:** A purchase should remain one purchase whether MIRA first learns about it from an email, a phone photo, a screenshot or the user saying what they bought.

**Requirement status:** `required`.

**Delivery/evidence:** normalized evidence/source-identity validation and idempotent evidence primitives are implemented/test-supported in the legacy evidence core; photo/manual/email ingestion and canonical dedupe rules are strongly `specified`. PR #31 contains an unmerged receipt-processing queue candidate, reference evidence only. MIRA 2.0 stock ChatGPT+Google receipt intake/provider readback remains unverified.

**Hard dependencies:** stable Receipt ID; canonical purchase/evidence authority; provenance/source identity; file retention when used; `ORDER-001`; `RECOVERY-002`.

**Legacy evidence:** category-C row 6; receipt ingestion/photo/classification policies; `asset_evidence.py` and tests; PR #31 candidate receipt-processing service.

**Acceptance / verification boundary:** Prove multiple evidence variants converge on one Receipt ID with retained provenance, ambiguity queues, and no OCR overwrite; then sandbox provider readback.

### `RECEIPT-002` — Searchable expandable purchase history and connected receipt graph

**Description:** MIRA exposes durable purchase history by Receipt ID with searchable line detail and links to evidence, assets, identifiers, manuals/specifications and relationships. Receipt-, asset- and identifier-origin queries return the same connected reality rather than separate ledgers; user-facing expandable views are projections.

**Requirement:** required. **Evidence:** `test_verified` deterministic connected graph; MIRA 2.0 Receipt Browser/provider readback unverified. **Dependencies:** `RECEIPT-001`, stable receipt/evidence/entity identities and relationship graph.

**Legacy evidence:** category-C row 7; receipt-browser policy; `asset_evidence.py` `query_graph`; graph tests.

### `SPEND-001` — Evidence-bounded monthly spending rollup

**Description:** MIRA summarizes a selected month from canonical receipt/email-detected evidence with one-count transaction dedupe, category totals, monthly total and unresolved ambiguity. It must state its evidence boundary and must not claim complete account spending unless a separately verified full financial authority exists.

**Requirement:** required. **Evidence:** specified/skill workflow; no dedicated deterministic monthly rollup suite located. **Dependencies:** `RECEIPT-001`, `RECEIPT-003`, one-count totals/allocations and coverage metadata.

**Verification boundary:** deterministic duplicate/mixed/revision/refund/coverage-label tests plus synthetic-month sandbox reconciliation.

### `RECEIPT-003` — Generic configurable receipt taxonomy and line classification

**Description:** Receipt classification is line-item based, supports generic/configurable categories, tags, cost owners, assets/projects and mixed receipts, and never hard-codes a private user's assets/merchants as universal defaults. Ambiguity is queued after reasonable evidence work rather than guessed.

**Requirement:** accepted / downstream prerequisite. **Evidence:** specification-level general taxonomy; no generic configurable classifier implementation sufficient for promotion. **Dependencies:** `RECEIPT-001`, stable line identity, classification queue and balanced allocations.

### `PAYMENT-001` — Expected merchant charge and settlement reconciliation

**Description:** MIRA tracks one payment case per Receipt ID/current merchant financial outcome, separate from purchase history. Supported states include Awaiting Settlement, Pending Match, Matched, Split Settlement, Overcharged, Undercharged, Pending Release, Settlement Contradiction, Refund/Reversal Expected, Resolved No Settlement and Ambiguous. Pending is not final; debit/credit direction and posted net are preserved.

**Requirement:** accepted / required for trustworthy financial lifecycle. **Evidence:** `test_verified` deterministic merchant-payment core; connected account/provider matching unverified. **Dependencies:** `RECEIPT-001`, expected merchant amount, stable payment identity, optional authorized account adapter, `ORDER-003`, `RECOVERY-002`.

### `REIMB-001` — Beneficiary allocation and household reimbursement reconciliation

**Description:** A purchase retains one Receipt ID/gross merchant total while allocations identify beneficiaries/assets. A separate Reimbursement ID records money expected/received with states such as Expected, Partially Received, Received, Waived or Disputed. Reimbursement is not merchant refund/revenue and changes net household cost only from supported reimbursement evidence.

**Requirement:** accepted. **Evidence:** strongly specified; no dedicated deterministic reimbursement engine/test suite located. **Dependencies:** `RECEIPT-001`, beneficiary identity, balanced allocations, incoming-payment evidence and separation from `PAYMENT-001`.

## Category C2 consistency findings

- Receipt evidence surfaces converge on one canonical transaction identity.
- Receipt graph queries are test-verified; UI/provider projection is separate.
- Receipt-derived spending is evidence-bounded rather than complete-account truth.
- Generic taxonomy and reimbursement remain explicit implementation gaps.
- Merchant settlement and reimbursement are different authorities.
- PR #31 receipt-processing code is unmerged reference evidence only.

## Final category-C direction features

### `SUB-001` — Optional subscription and free-trial tracking

**Description:** MIRA may offer an explicitly enabled service for recurring subscriptions and free trials that records stable commitment identity, merchant/service, supported recurring amount/cadence, trial or renewal/cancel-by timing when evidenced, current status, source/provenance and user decisions. The service must deduplicate repeated email/receipt/account evidence, must not create a separate scheduled automation per subscription, and must never automatically cancel, contact a merchant, or silently activate merely because recurring-looking evidence appears. It can surface an upcoming renewal/trial decision through an existing compatible MIRA control cycle when the user has enabled the capability.

**Why it exists / user outcome:** A user can avoid surprise renewals without turning every recurring charge into another autonomous robot job or letting MIRA cancel something based on a marketing email it misunderstood.

**Requirement status:** `proposed / optional`; the forensic ledger records a previous paused automation, and this C3 audit found no newer direct requirement that promotes it to a current universal or active-milestone feature.

**Delivery/evidence:** historical automation concept/requirement evidence only; no current audited executable subscription/free-trial engine or MIRA 2.0 integration was found. Treat as `desired/specification pending` rather than implemented.

**Hard dependencies:** explicit per-user service activation; stable subscription identity; `RECEIPT-001` and/or `FIN-001` evidence when used; `MAIL-002` for any contact proposal; existing scheduler/control-cycle semantics rather than per-subscription jobs.

**Enables:** opt-in renewal/trial warnings and recurring-commitment review.

**Legacy evidence:** category-C row 11 in `docs/feature-ledger-2026-08-24.md` records a proposed/previous paused automation and explicitly says it must not resurrect silently. No narrower current implementation evidence was located in the audited repository searches.

**Acceptance / verification boundary:** Before implementation credit, define activation/config/state and add deterministic fixtures for duplicate evidence, renewal-date precedence, cancelled/ended state, free-trial conversion, missing/ambiguous amount or date, and no-auto-contact/cancellation. Integration verification requires synthetic provider evidence/readback with the service disabled by default.

**Compatibility notes:** This remains valid later work but is not a prerequisite for the current stock ChatGPT+Google→Android vertical slice unless dependency ranking later proves otherwise.

---

### `FIN-001` — Complete connected financial-account ingestion and reconciliation

**Description:** MIRA may eventually connect authorized financial accounts to provide account-grounded transactions, balances/coverage metadata and reconciliation beyond receipt/email evidence. This capability is explicitly broader than `PAYMENT-001`: it must preserve stable account/transaction identities, posted versus pending state, debit/credit direction, coverage/sync boundaries, transfers versus real income/outflow, duplicate provider observations, and enough provenance to explain conclusions without copying sensitive account contents into public Git. Account data can corroborate purchases, merchant settlement, reimbursements, subscriptions and spending, but it does not replace canonical Receipt IDs or mutate purchase history merely because a bank descriptor differs.

**Why it exists / user outcome:** Complete financial answers require actual account coverage. Receipt/email evidence can tell MIRA a lot, but it cannot honestly tell the user their total spending, cash flow, balances or every recurring charge unless the relevant accounts are actually connected and synchronized.

**Requirement status:** `proposed / infrastructure-deferred direction` in the forensic ledger. It is not required for the current Native MIRA stock vertical slice and is not implemented in the audited repository.

**Delivery/evidence:** `not_present` as a complete MIRA repository capability. The narrower `PAYMENT-001` deterministic merchant reconciler is test-verified but does not constitute account ingestion. No repository search evidence justified promotion of complete financial ingestion to implemented/integration/live status.

**Hard dependencies:** explicit user authorization/consent; provider connector abstraction; private account identity mapping; sync/coverage/readback metadata; transaction dedupe/reconciliation; privacy/secrets handling; `RECOVERY-002`; clear separation from `RECEIPT-*`, `PAYMENT-001`, `REIMB-001` and `SPEND-001`.

**Enables:** complete-account spending/cash-flow views, account-backed recurring-charge detection, balance/liability/portfolio modules when separately specified, and stronger payment/reimbursement reconciliation.

**Legacy evidence:** category-C row 12 records credit-card linkage/complete financial ingestion as proposed/infrastructure and not present; existing receipt/payment policies explicitly treat connected-account evidence as optional supporting evidence rather than proof that a complete connector already exists.

**Acceptance / verification boundary:** A future bounded implementation must prove explicit connection scope, account enumeration/readback, sync/coverage limits, pending-vs-posted handling, stable transaction dedupe, transfer classification, provider failure isolation and no-secret/public-Git leakage. Complete-spending claims require documented coverage across all relevant connected accounts. No money movement or external financial action is implied by read/analysis connectivity.

**Compatibility notes:** `FIN-001` may later improve `SPEND-001`, `PAYMENT-001`, `REIMB-001` and `SUB-001`, but those features retain their own identities/semantics and must not become hidden projections of a finance provider.

## Category C consistency result

Category C is fully audited through row 12.

- Evidence adapters (`MAIL-*`, files/photos, future `FIN-001`) do not replace canonical Receipt/Order identity.
- `ORDER-*` owns fulfillment/lifecycle semantics; active Shipments remains a projection rather than durable purchase history.
- `RECEIPT-*` owns one-count purchase identity/evidence/history/classification.
- `SPEND-001` is explicitly evidence-bounded unless complete account coverage is independently verified.
- `PAYMENT-001` owns merchant expected settlement/refund outcome; `REIMB-001` owns money expected/received from another beneficiary/payer. Neither is the other.
- `SUB-001` is optional/proposed and must not silently resurrect a paused historical automation.
- `FIN-001` is deferred complete-account infrastructure, not evidence that MIRA currently has complete finances.
- Open implementation gaps remain explicitly ranked: `ORDER-STALE-001`, `SPEND-ROLLUP-001`, `RECEIPT-TAXONOMY-001`, and `REIMB-CORE-001`; optional future subscription/full-finance implementation must be separately scoped.
- No category-C feature receives MIRA 2.0 integration/live status from legacy Google or account connections.

## Audit status

- Category A is complete through `M2-G0-002D`.
- Category B is complete through `M2-G0-003B`.
- `M2-G0-004A` audited category-C rows 1-5.
- `M2-G0-004B` audited category-C rows 6-10.
- `M2-G0-004C` audited category-C rows 11-12 and completed the category-C consistency pass.
- **Category C is complete.**
- The next audit category is D: assets, fitment, inventory, shopping and household storage, which must begin with a bounded packet around stable asset identity/fitment and evidence foundations.
