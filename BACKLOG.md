...
| 6a | `FEATURE-ALIGN-001` | HARDENING | Implement `DEV-007` packet-to-feature alignment verification. Immediate policy requires every `CURRENT_WORK` packet to record primary features, required user-visible behavior, preserved invariants and deferred related features before implementation and before merge; automate drift checks where practical from `DEV-005` without pretending semantic product judgment is fully machine-solvable. | DEV-007,DEV-005,DEV-002,DEV-003 | policy active; automation queued |
...
| `FIRSTBOOT-CORE-001` | PREREQUISITE | Port/test `ONBOARD-003`: exactly four kickoff questions with fixed MIRA name omitted: (1) authoritative IANA timezone, (2) broad life/work/study/caregiving pattern, (3) biggest remembering/organizing/deciding/planning/follow-through goals, (4) appointment-reminder interest + preferred Calendar auto-sync intent. Persist durable resumable Interview Ledger; after four, tell the user they may ask MIRA anytime to continue the interview with additional questions to improve function and introduce integrated MIRA Studio + optional sharing. Evidence-first reuse remains mandatory; preferences/permissions are never inferred. | ONBOARD-002,ONBOARD-003,canonical onboarding state | queued; legacy contract preserved but requirement refined 2026-08-29 |
| `DISCOVERY-CORE-001` | HARDENING | Structured discovery without recommendation-driven activation, including current AI-use/friction discovery and inspection of accessible conversation/files/connected evidence before asking the user to rebuild history. MIRA must support user-invoked continuation of interview questions at any later time while preserving the Interview Ledger. | ONBOARD-003,ONBOARD-004,SERVICE-001,RUNTIME-ROUTER-001 | queued; legacy question banks remain source evidence |
...
- **Feature alignment refinement (2026-08-29):** `DEV-007` now requires every packet to read the canonical feature set before implementation and before merge, record required user-visible behavior/preserved invariants/deferred related features in `CURRENT_WORK`, and reject locally green solutions that structurally contradict accepted product features. `FEATURE-ALIGN-001` owns later automation; the policy is effective immediately.
...
- **M2-M1 command-boundary finding (2026-08-29):** provider-neutral sequencer PR #54 and synthetic Workspace queued-writer PR #55 are merged/green. Android is now blocked on live isolated Google worker proof and then Android client enrollment/sync, not on inventing a concurrency architecture from scratch.
...
- The audited onboarding interview remains preserved as semantic/source evidence, but its current requirement is refined: MIRA's name is fixed and is not asked; the fourth kickoff question is appointment-reminder help + preferred Calendar auto-sync intent; the closing orientation explicitly offers user-invoked additional interview questions and introduces MIRA Studio. `FIRSTBOOT-CORE-001` remains queued because the full Interview Ledger engine is not yet ported.
- Appointment capture is now explicit under `CAL-008`/`APPOINTMENT-INTAKE-001`: inbound email, user-supplied image/photo, and user text can become provenance-bound canonical appointment/provider state, including durable provider specialty/type. Preferred Calendar sync remains a separate verified projection so evidence intake does not silently equal provider write success.
