...
| `CALENDAR-PROJECTION-GOOGLE-001` | PREREQUISITE | Stock-ChatGPT Google Calendar connector capability adapter/proof: exact create/readback and stable projection identity where supported; unsafe update must fail closed when the exposed mutation surface cannot enforce the core's provider-version precondition. Live proof requires an isolated synthetic Calendar rather than primary/family production state. | CAL-006,CAL-007,CALENDAR-PROJECTION-001A,Google Calendar connector capability | active in `M2-M0-023`; read-only discovery found no isolated synthetic Calendar and no exposed ETag/If-Match update precondition |
| `CALENDAR-PROJECTION-GOOGLE-UPDATE-001` | BLOCKER | Strong Google Calendar guarded-update path with atomic provider-version/ETag precondition and exact readback, preserving the same canonical projection identity. | CALENDAR-PROJECTION-GOOGLE-001,provider mutation surface with atomic version precondition | queued; current stock connector update action exposes no ETag/If-Match precondition |
...
- `CALENDAR-PROJECTION-GOOGLE-001` is the single active work item in `M2-M0-023`. Read-only connector discovery found no isolated synthetic Calendar among visible calendars and found no exposed ETag/If-Match precondition on generic event updates. Existing personal/family Calendars remain protected and unavailable as development fixtures.
