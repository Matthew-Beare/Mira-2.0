...
| `SHOP-CORE-001` | PREREQUISITE | Deterministic shopping-intent reconciliation. | RECEIPT-001 | active in `M2-M0-018`; bounded canonical shopping-intent + explicit receipt reconciliation implementation |
...
- `SHOP-CORE-001` is the one active work packet in `M2-M0-018`; it keeps current procurement intent separate from receipt/purchase history and adds only explicit conservative receipt reconciliation.
