...
| `OPS-BRIEF-VSLICE` | VERTICAL | First real task-centered no-app MIRA Ops Brief from canonical MIRA 2.0 state with deterministic local slots/run identity, immutable composed checkpoint, one-action-per-task rendering, explicit non-delivery truth and optional bounded progressive-discovery prompt. | STORE-001,TASK-001,TASK-002,OPS-003,OPS-004,RECOVERY-001 | active in `M2-M0-011`; implementation/tests + isolated Google task/run exact readback complete in PR #63, latest-head closeout/merge pending |
...
- `DISCOVERY-CORE-001` remains partial: progressive discovery is test-verified and PR #63 adds repeated same-unanswered-topic daily prompting without silent advancement; broader evidence-aware AI-use/friction/history discovery remains unfinished.
- `OPS-BRIEF-VSLICE` is active in M2-M0-011 with its broad service-composition prerequisite removed for the bounded same-user task brief. Generic service composition remains queued for richer multi-service briefs rather than blocking this first useful vertical.
