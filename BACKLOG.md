...
|---|---|---|---|---|
| `AUDIT-A` | PREREQUISITE | Feature Audit Slice A: brief/time/tasking/operational-state feature reconstruction. | G0-001 | **in progress**; A1 complete, A2 next |
...
| `AUDIT-A2` | PREREQUISITE | Audit category-A behaviors 6-10: generic context pairs; job/duties context recommendation; active trip separation; multi-leg route/runtime/location/ETA inference; ROAD severe-weather vs HOME weather. | AUDIT-A1 | **next packet `M2-G0-002B`** |
| `AUDIT-A3` | PREREQUISITE | Audit remaining category-A behaviors 11-16: mileage/gross, tracker authority, task hierarchy, next-action evidence, Run Log/recovery/circuit breaker, optional-module failure isolation. | AUDIT-A2 | queued |
...
| `OPS-BRIEF-VSLICE` | VERTICAL | Generate/deliver one real MIRA Ops Brief from canonical MIRA 2.0 state. | core + delivery prerequisites | provisional; depends on audited `OPS-*` foundation |
...
- `OPS-005` must remain separate from generic context pairs and active-trip forcing. Those are intentionally queued in `AUDIT-A2` rather than silently absorbed.
