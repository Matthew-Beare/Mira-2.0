...
- `CALENDAR-PROJECTION-GOOGLE-001` remains the active work item in `M2-M0-023`, now with native default-Personal implementation/test/release evidence. Exact implementation head `d0a3c9a2d2b1ab2de59aa2fd8fe3419907b48903` passed CI `33437591056` (351 Python + 30 Apps Script tests). Fresh Calendar inventory still shows no isolated writable test Calendar, so live event write/readback remains unverified and protected Primary/Family Calendars remain unavailable as development fixtures. `CALENDAR-PROJECTION-GOOGLE-UPDATE-001` is therefore HARDENING for concurrent/shared writers, not a blocker for ordinary Personal single-writer activation.
