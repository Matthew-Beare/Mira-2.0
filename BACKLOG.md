...
| `BACKUP-CORE-001` | PREREQUISITE | Implement `BACKUP-001` backup/restore lifecycle and independent restore verification. | BACKUP-001,RECOVERY-002,STORE-ADAPTER-001A | active in `M2-M0-020`; provider-neutral full snapshot/digest/isolated-restore parity core |
...
- `BACKUP-CORE-001` is the one active work packet in `M2-M0-020`; it is bounded to provider-neutral full snapshot, deterministic digest, isolated restore and parity verification, leaving provider archive/encryption/retention/scheduling/migration concerns unfinished.
