...
| `CALENDAR-PROJECTION-001` | PREREQUISITE | Umbrella preferred-Calendar projection capability. Split into provider-neutral projection semantics and provider-specific capability/readback proof rather than one oversized packet. | CAL-007,CAL-006,RECOVERY-002 | split; children below |
| `CALENDAR-PROJECTION-001A` | PREREQUISITE | Provider-neutral source-linked Calendar projection core: stable projection identity, deterministic timed-event material, replay/source/provider conflict semantics, adapter capability contract and exact synthetic readback. No real provider mutation. | CAL-007,CAL-005,RECOVERY-002,STORE-001 | complete in PR #77 at `fcc81f0e9d9510d05406acd9068d9ca4ee016806`; exact-head CI `33427269214` and post-merge `main` CI `33427385919` green; synthetic/provider-neutral readback only |
| `CALENDAR-PROJECTION-001B` | PREREQUISITE | Preferred-Calendar provider adapter/capability proof with exact write/readback for the selected Google, Microsoft/Outlook/M365 or Apple/iCloud lane; unsupported lanes fail closed and are never silently substituted. | CAL-006,CAL-007,CALENDAR-PROJECTION-001A,provider Calendar write/readback | active in `M2-M0-023`; bounded Google adapter implementation/test slice first, while live provider proof remains required before this work is complete |
...
- `CALENDAR-PROJECTION-001A` is completed by PR #77 at merge `fcc81f0e9d9510d05406acd9068d9ca4ee016806`; exact-head CI `33427269214` and post-merge `main` CI `33427385919` are green. This evidence is provider-neutral/synthetic only and does not claim a live Google, Microsoft or Apple Calendar write.
- `CALENDAR-PROJECTION-001B` is active in `M2-M0-023` as a bounded Google adapter implementation/test slice. The connected Google account currently exposes only primary, Family and Holidays calendars and the connector cannot create a separate Calendar, so live proof is intentionally deferred until an isolated MIRA test Calendar exists; protected personal Calendar state is not a development fixture.
