...
- `LOCATION-STATE-001` is the one active packet. It implements `INV-001` participation by reusing the asset Entity UUID and `LOC-001` intended-versus-observed hierarchical location state while explicitly stopping before movement-event history, scanning and broad inventory query.
