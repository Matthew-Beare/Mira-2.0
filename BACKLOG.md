...
| `MOVEMENT-CORE-001` | PREREQUISITE | Replay-safe movement/observation events; never collapse intended/observed location. | INV-001,IDENT-001,LOCATION-STATE-001 | active in `M2-M0-017`; event-first/projection-second recovery-safe movement semantics in implementation |
...
- `MOVEMENT-CORE-001` is the one active packet in `M2-M0-017`; it establishes replay-safe explicit observation/movement event semantics and observed-state projection while scanning/capture remains separate unfinished work.
