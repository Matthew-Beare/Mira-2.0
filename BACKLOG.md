...
| `INVENTORY-QUERY-001` | HARDENING | Canonical inventory query projection. | INV-001,LOC-001,ASSET-003,LOCATION-STATE-001 | active in `M2-M0-016`; PR #69 candidate; direct tests, complete no-app instruction/release guards, and fresh isolated Google read-only query/zero-write readback verified; final exact-head CI/merge pending |
...
- `INVENTORY-QUERY-001` is the one active packet in `M2-M0-016`. PR #69 now has direct tests, complete no-app instruction/release guards, and fresh isolated Google read-only query/zero-write provider proof; exact-head CI/merge closeout remains before completion.
