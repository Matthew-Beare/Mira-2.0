...
| `APPOINTMENT-INTAKE-001` | VERTICAL | Umbrella evidence-first appointment capture from inbound email, user-supplied photo/image, or user text through provenance/confidence extraction, provider + appointment reconciliation, materially ambiguous Needs Review, dedupe and optional capability-gated preferred-Calendar projection. Outbound provider contact remains separately governed by `MAIL-002`. | CAL-008,APPOINTMENT-IDENTITY-001,CALENDAR-PROJECTION-001 | split/partial; provider-neutral core complete, source-to-extraction user flows remain unfinished |
...
- Appointment capture is explicit under `CAL-008`/`APPOINTMENT-INTAKE-001`: the provider-neutral core is complete, while no-app direct text/image extraction and Gmail source integration remain explicit child work. Preferred Calendar sync remains a separate capability/readback boundary so evidence intake does not silently equal provider write success.
