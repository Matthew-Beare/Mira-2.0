...
### Wave 2 — Google Workspace-first shared API + stock ChatGPT proof (M2-M0)
...
| 9 | `API-DEPLOYMENT-001` | PREREQUISITE | Umbrella ordinary-user-compatible HTTPS deployment proof for `API-CORE-001`. The default Personal path must require no self-hosted server or terminal; advanced managed/self-hosted profiles remain interchangeable behind the same API/Authority contracts. | API-CORE-001,CORE-SYNTHETIC-ROUNDTRIP | split; default Workspace child now blocks M2-M0 |
| 9a | `API-DEPLOYMENT-001A` | PREREQUISITE | **Default Personal first-run:** copyable Google Workspace starter using Sheets as initial structured MIRROR authority and bound Apps Script as the lightweight HTTPS execution boundary. Setup must be browser-only with no Cloud Run project, Linux server, SQL server, terminal, Cloudflare/WireGuard requirement, or paid OpenAI API. Preserve `API-001`/`AUTH-001`/`STORE-001` semantics so later Linux/SQL migration is a backend cutover, not a product rewrite. | API-CORE-001,CORE-SYNTHETIC-ROUNDTRIP,GOOGLE-STORE-ADAPTER-001,DATA-SANDBOX,ONBOARD-002 | selected active target for `M2-M0-006` |
...
| 10 | `CHATGPT-API-CLIENT-001` | PREREQUISITE | Prove the minimal supported stock-ChatGPT path to the default deployed shared API using same-user scoped credentials/actions and no direct canonical-provider mutation. Full MIRA service onboarding is not required for this proof. | API-DEPLOYMENT-001A | queued immediately after Workspace endpoint proof |
| 11 | `CORE-ROUNDTRIP` | VERTICAL | **M2-M0:** stock ChatGPT create/read/mutate/replay/read-back one Google-backed canonical MIRROR entity through the default Google Workspace deployment path. | CORE-SYNTHETIC-ROUNDTRIP,DATA-SANDBOX,GOOGLE-STORE-ADAPTER-001,API-DEPLOYMENT-001A,CHATGPT-API-CLIENT-001 | provisional target |

### Wave 3 — Android shared-state proof (M2-M1)

| Rank | Work ID | Class | Work | Dependencies | Status |
|---:|---|---|---|---|---|
| 12 | `ANDROID-CLIENT-CORE-001` | PREREQUISITE | MIRA 2.0 Android shared-API client: API compatibility, enrollment/session identity, revocation, OS-protected credentials, bounded reads, canonical commands, replay-safe offline queue, reconnect/cursor sync and server conflict/readback handling. | CLIENT-ANDROID-001,API-CORE-001 | queued; direct provider paths prohibited |
| 13 | `ANDROID-SYNC` | VERTICAL | **M2-M1:** Android reads/mutates the exact M2-M0 canonical entity through `API-001`; stock ChatGPT reads the Android mutation back from the same authority. | CORE-ROUNDTRIP,ANDROID-CLIENT-CORE-001 | provisional target |
...
| `VOICE-CLIENT-001` | LATER | Optional voice query/command surface over shared API with consequential-action confirmation. | VOICE-001,API-CORE-001 | deferred |
...
| `AUTHORITY-MIGRATION-001` | PREREQUISITE | Staged provider/backend cutover without dual writable masters; parity/readback/recovery/rollback. This is the protected upgrade path from the default Google Workspace backend to Linux/SQL/managed backends. | AUTH-001,STORE-001,BACKUP-001,AUTHORITY-REGISTRY-001,STORE-ADAPTER-001A | queued; portability invariant applies during `API-DEPLOYMENT-001A` |
...
| `MIRA-SKILL-001` | PREREQUISITE | Full provider-neutral MIRA orchestration skill. | ONBOARD-007,DIST-STARTER-001,SOURCE-GATES-001,RUNTIME-ROUTER-001,SERVICE-COMPOSE-001 | queued; not required for the minimal M2-M0 Workspace endpoint proof |
...
| `GOOGLE-BOOTSTRAP-001` | PREREQUISITE | Full deterministic Personal Google blueprint/plan/verifier across selected Workspace services. | PROVIDER-003,MIRA-SKILL-001,PROVIDER-ONBOARD-001,SOURCE-GATES-001 | queued after `API-DEPLOYMENT-001A`; baseline first-run must already be easy/browser-only |
| `NONTECH-INSTALL-001` | PREREQUISITE | Full browser-only ordinary-user installation, upgrade/recovery and exact readback. | ONBOARD-006,DIST-STARTER-001,SOURCE-GATES-001,MIRA-SKILL-001,PROVIDER-ONBOARD-001 | queued hardening after baseline; `API-DEPLOYMENT-001A` must not require terminal fallback |
...
| `DIST-STARTER-001` | PREREQUISITE | Deterministic MIRA 2.0 starter/distributions from one source SHA. | DIST-002,ONBOARD-002,clean source lineage | queued |
| `SKILL-BUILDER-001` | ENHANCEMENT | Bounded private custom-feature workflow. | DEV-004,DEV-001,DEV-002,FEATURE-REGISTRY-001,SOURCE-GATES-001 | queued |
...
| `STARTER-SANITIZE-001` | PREREQUISITE | Starter privacy/history audit gates. | ONBOARD-002,DIST-002 | queued |
| `FIRSTBOOT-CORE-001` | PREREQUISITE | ≤4-question kickoff + resumable Interview Ledger/Minimum Useful Setup. | ONBOARD-002,canonical onboarding state | queued |
| `DISCOVERY-CORE-001` | HARDENING | Structured discovery without recommendation-driven activation. | ONBOARD-003,SERVICE-001,RUNTIME-ROUTER-001 | queued |
...
- **Superseded priority finding (2026-08-29):** the earlier decision that browser-only installation / Personal Google first-run could wait until after M2-M0 was wrong for the intended product. The default M2-M0 path now requires the minimal Google Workspace browser-only deployment in `API-DEPLOYMENT-001A`; full Gmail/Calendar/scheduler/service onboarding still does not block the minimal entity proof.
...
- `API-DEPLOYMENT-001` is now an umbrella: `001A` is the ordinary-user Google Workspace baseline; `001B` preserves the already-built Cloud Run advanced profile. `CHATGPT-API-CLIENT-001` and `CORE-ROUNDTRIP` depend on `001A` for M2-M0.
