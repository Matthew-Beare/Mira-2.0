...
- **Related invariants/features:** `API-001`, `AUTH-001`, `STORE-001`, `RECOVERY-002`, `PROVIDER-002`, `ONBOARD-006`
- **Repository:** `Matthew-Beare/Mira-2.0`
- **Branch:** `work/m2-m1-002-android-client-core`
- **Base SHA:** `acb690e66f42bdf02d9030dc0ead0d3d42195e57`
- **Current head SHA:** activation checkpoint pending
- **Status:** active; implementation has not begun at this checkpoint
...
2. Enrollment returns opaque credential material only at creation; persisted session state retains only a cryptographic verifier, never the raw credential.
...
7. Tests prove successful enrollment/authentication, wrong-secret denial, revocation denial/idempotency, exact grant reconstruction, and absence of raw credential material from durable session snapshots.
...
- CI run `33681423055` was verified `success` on that exact main SHA.
...
1. Commit this packet activation checkpoint on `work/m2-m1-002-android-client-core`.
2. Implement the scoped enrollment/session registry against existing `Grant` / `AuthenticatedPrincipal` semantics.
3. Add deterministic tests for issuance, verifier-only persistence, authentication, revocation, and exact grant reconstruction.
4. Run/verify repository gates; update packet evidence and only required lifecycle docs.
5. Push, remotely verify exact head and CI, then stop rather than expanding into offline sync or Android UI.
