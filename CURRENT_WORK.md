...
`M2-M1-046` is merged to `main` at `8ed16f9e2b3e2672ff7149488ba0a7d28a4a0222`. Remote `main` was read back at that SHA and the post-merge Trusted Runner Gate completed successfully. The previous `CURRENT_WORK.md` checkpoint was stale because it still described PR/merge work as pending after the merge; this packet corrects that recovery state before selecting more work.
...
2. Read connected provider history coverage directly, including earliest accessible history and freshness/coverage limitations. **PENDING.**
3. Identify every accessible provider transaction older than the current canonical projection lower bound by stable provider identity. **PENDING.**
4. Reconcile that entire accessible pre-projection set into the existing canonical finance model or record an exact source-bounded exception; do not silently ignore rows because they predate MIRA 2.0. **PENDING.**
5. Preserve old historical evidence/provenance; no destructive cleanup or artificial cutoff. **PENDING verification.**
...
7. Duplicate Event IDs, Evidence IDs, Entity IDs and Relation IDs remain zero; broken relation endpoints remain zero after reconciliation. **PENDING.**
8. Canonical finance surfaces state the actual historical coverage boundary and unavailable gaps truthfully. **PENDING.**
9. `FIN-CANON-AUDIT-001` closes only if the accessible historical boundary is fully reconciled and verified. **PENDING.**
10. Public Git contains sanitized evidence only. **PENDING closeout verification.**
...
## Five-line customer status

Objective: Reconcile all trustworthy accessible historical finance data into canonical MIRA state.
Progress: Current evidence lanes are reconciled; the remaining gap is older provider history outside the current projection.
Deliverable: A finance model with a truthful complete accessible-history boundary and no hidden pre-MIRA transaction gap.
Expected delivery: UNKNOWN until the pre-projection transaction set is measured directly.
Blocker: None currently; next action is provider-history measurement and reconciliation.
...
1. Read connected finance account coverage and determine the earliest accessible posted transaction for every relevant account.
2. Read the current canonical finance projection lower bound.
3. Compute the exact pre-projection reconciliation set using stable provider transaction identity.
4. Reconcile that set into the existing canonical ledger/model with no duplicate economic effect.
5. Link historical evidence only where supported; preserve unresolved evidence and unavailable gaps.
6. Rerun duplicate/event/evidence/entity/relation and broken-endpoint integrity gates.
